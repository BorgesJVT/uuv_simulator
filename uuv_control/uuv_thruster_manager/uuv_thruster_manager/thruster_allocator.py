...
import yaml
from uuv_thruster_manager import ThrusterManager
...
class ThrusterAllocatorNode(ThrusterManager):
    ...
    def __init__(self):
        ...
        self.input_sub = self.create_subscription(Wrench, '/thruster_manager/input', self.input_callback, 1)
        ...
        # message
        self.input_stamped_sub = self.create_subscription(WrenchStamped,'thruster_manager/input_stamped', self.input_stamped_callback, 10)
        self.thruster_info_service = self.create_service(ThrusterManagerInfo, 'thruster_manager/get_thrusters_info', self.get_thruster_info)
        self.curve_calc_service = self.create_service(GetThrusterCurve, 'thruster_manager/get_thruster_curve', self.get_thruster_curve)
        self.set_thruster_manager_config_service = self.create_service(SetThrusterManagerConfig, 'thruster_manager/set_config', self.set_config)
        self.get_thruster_manager_config_service = self.create_service( GetThrusterManagerConfig, 'thruster_manager/get_config', self.get_config)

        rate = self.create_rate(self.config['update_rate'])
        
        #while True: #not rclpy.is_shutdown():
           # if self.config['timeout'] > rclpy.duration.Duration(0):
                # print("ALAGOANO É TUDO GALADO")
                # If a timeout is set, zero the outputs to the thrusters if
                # there is no command signal for the length of timeout

            #    if self.get_clock().now() - self.last_update > self.config['timeout'] :
    ...
    def input_callback(self, msg):
        ...
        self.get_logger().info('input callback')
        if not self._ready:
            return
    ...
    def input_stamped_callback(self, msg):
        ...
        self.get_logger().info('input stamped callback')
