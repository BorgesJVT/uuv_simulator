...
class VelocityControllerNode:
    def __init__(self):
        super(VelocityControllerNode, self).__init__()
        ...
        # ROS infrastructure
        self.sub_cmd_vel = self.create_subscription('cmd_vel', geometry_msgs.Twist, self.cmd_vel_callback)
        self.sub_odometry = self.create_subscription('odom', Odometry, self.odometry_callback)
        self.pub_cmd_accel = self.create_publisher('cmd_accel', geometry_msgs.Accel, 10)
...
if __name__ == '__main__':
    ...
    self.init_node('velocity_control')
