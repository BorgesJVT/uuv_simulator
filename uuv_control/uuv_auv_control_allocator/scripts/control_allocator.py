...
import os
import casadi
from uuv_auv_control_allocator import ActuatorManager
...
class AUVControlAllocator(ActuatorManager):
    def __init__(self):
        ...
        if self.has_parameter('~output_dir'):
            self.output_dir = self.get_parameter_value('~output_dir')
        ...
        self.update_rate = self.get_parameter_value('~update_rate', 10)

        self.timeout = self.get_parameter_value('~timeout', -1)

        if self.update_rate <= 0:
            raise self.get_logger().warn('Update rate must be a positive number')
        ...
        self.sub = self.create_subscription('control_input', AUVCommand, self.control_callback)
        ...
        while not self.is_shutdown():
            ...
            if self.timeout > -1:
                if self.get_clock().now() - self.last_update > self.timeout:
                    self.input_surge_speed = 0.0
                    self.input_tau = np.zeros(6)            
...
if __name__ == '__main__':
    self.init_node('auv_control_allocator')
