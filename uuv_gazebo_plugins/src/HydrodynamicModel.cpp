...
HydrodynamicModel::HydrodynamicModel(sdf::ElementPtr _sdf,
    gazebo::physics::LinkPtr _link) : BuoyantObject(_link)
{
  ...
  {
    cob = gazebo::Str2Vector(_sdf->Get<std::string>("center_of_buoyancy"));
                  ...
                  : HydrodynamicModel(_sdf, _link)
{
  ...
  if (modelParams->HasElement("added_mass"))
    addedMass = gazebo::Str2Vector(modelParams->Get<std::string>("added_mass"));
  else
  ...
  if (modelParams->HasElement("linear_damping"))
    linDampCoef = gazebo::Str2Vector(modelParams->Get<std::string>("linear_damping"));
  else
  ...
  if (modelParams->HasElement("linear_damping_forward_speed"))
    linDampForward = gazebo::Str2Vector(
  ...
  if (modelParams->HasElement("quadratic_damping"))
    quadDampCoef = gazebo::Str2Vector(
  ...
  double _time, const ignition::math::Vector3d &_flowVelWorld)
{
  ...
  if (!std::isnan(tau.norm()))
  {
    ...
    ignition::math::Vector3d hydForce =
      this->FromNED(gazebo::Vec3dToGazebo(tau.head<3>()));
    ignition::math::Vector3d hydTorque =
      this->FromNED(gazebo::Vec3dToGazebo(tau.tail<3>()));
  ...
  if ( this->debugFlag )
  {
    ...
    this->StoreVector(UUV_DAMPING_FORCE, gazebo::Vec3dToGazebo(damping.head<3>()));
    this->StoreVector(UUV_DAMPING_TORQUE, gazebo::Vec3dToGazebo(damping.tail<3>()));

    this->StoreVector(UUV_ADDED_MASS_FORCE, gazebo::Vec3dToGazebo(added.head<3>()));
    this->StoreVector(UUV_ADDED_MASS_TORQUE, gazebo::Vec3dToGazebo(added.tail<3>()));

    this->StoreVector(UUV_ADDED_CORIOLIS_FORCE, gazebo::Vec3dToGazebo(cor.head<3>()));
    this->StoreVector(UUV_ADDED_CORIOLIS_TORQUE, gazebo::Vec3dToGazebo(cor.tail<3>()));
  }
}
                                          ...
                                          Eigen::Matrix6d &_Ca) const
{
  ...
  Eigen::Matrix3d Sa = -1 * gazebo::CrossProductOperator(ab.head<3>());
  _Ca << Eigen::Matrix3d::Zero(), Sa,
         Sa, -1 * gazebo::CrossProductOperator(ab.tail<3>());
}
...
bool HMFossen::GetParam(std::string _tag, double& _output)
{
  ...
  if (!_tag.compare("volume"))
    ...
    _output = this->boundingBox.Size()[2];
  else if (!_tag.compare("bbox_width"))
    _output = this->boundingBox.Size()[1];
  else if (!_tag.compare("bbox_length"))
    _output = this->boundingBox.Size()[0];
                   ...
                   : HMFossen(_sdf, _link)
{
  ...
  else
  {
    gzmsg << "HMSphere: Using the smallest length of bounding box as radius"
          << std::endl;
    this->radius = std::min(this->boundingBox.Size()[0],
                            std::min(this->boundingBox.Size()[1],
                                     this->boundingBox.Size()[2]));
  }
                       ...
                       : HMFossen(_sdf, _link)
{
  ...
  else
  {
      gzmsg << "HMCylinder: Using the biggest length of bounding box as length"
            << std::endl;
      this->length = std::max(this->boundingBox.Size()[0],
                              std::max(this->boundingBox.Size()[1],
                                       this->boundingBox.Size()[2]));
  }
  ...
  else
  {
    gzmsg << "HMCylinder: Using the direction of biggest length as axis"
          << std::endl;
    double maxLength = std::max(this->boundingBox.Size()[0],
                                std::max(this->boundingBox.Size()[1],
                                         this->boundingBox.Size()[2]));
    if (maxLength == this->boundingBox.Size()[0])
      this->axis = "i";
    else if (maxLength == this->boundingBox.Size()[1])
      this->axis = "j";
    else
      this->axis = "k";
  }
